...
public static class MineradioDesktopNativeIconLayerGuard {
  ...
  private static System.Collections.Generic.List<ShieldRect> _requestedShields =
    new System.Collections.Generic.List<ShieldRect>();
  private static bool _shieldRequestPending;
  private static long _shieldRequestSequence;
  private static bool _shieldRegionApplied;
  ...
  private static System.Collections.Generic.List<ShieldRect> ParseShieldRects(string payload) {
    System.Collections.Generic.List<ShieldRect> rects = new System.Collections.Generic.List<ShieldRect>();
    if (String.IsNullOrWhiteSpace(payload)) return rects;
    string[] entries = payload.Split(new char[] { ';' }, StringSplitOptions.RemoveEmptyEntries);
    foreach (string entry in entries) {
      if (rects.Count >= 64) break;
      ...
      int width;
      int height;
      if (!Int32.TryParse(fields[0], out x) || !Int32.TryParse(fields[1], out y)
          || !Int32.TryParse(fields[2], out width) || !Int32.TryParse(fields[3], out height)
          || width <= 0 || height <= 0) continue;
      ShieldRect rect = new ShieldRect();
      rect.x = x;
      rect.y = y;
      rect.width = width;
      rect.height = height;
  ...
  private static void ApplyListViewShieldRegion() {
    if (!BoundTargetIdentityMatches())
      throw new InvalidOperationException("DESKTOP_ICON_SHIELD_IDENTITY_UNCERTAIN");
    System.Collections.Generic.List<ShieldRect> shields;
    lock (StateLock) {
      shields = new System.Collections.Generic.List<ShieldRect>(_requestedShields);
    }
    if (shields.Count == 0) {
      if (_shieldRegionApplied && SetWindowRgn(_listView, IntPtr.Zero, true) == 0)
        throw new InvalidOperationException("DESKTOP_ICON_SHIELD_CLEAR_FAILED");
      _shieldRegionApplied = false;
      return;
    }
    RECT windowRect;
    if (!GetWindowRect(_listView, out windowRect))
      throw new InvalidOperationException("DESKTOP_ICON_SHIELD_BOUNDS_FAILED");
    int windowWidth = Math.Max(1, windowRect.right - windowRect.left);
    int windowHeight = Math.Max(1, windowRect.bottom - windowRect.top);
    IntPtr region = CreateRectRgn(0, 0, windowWidth, windowHeight);
    if (region == IntPtr.Zero)
      throw new InvalidOperationException("DESKTOP_ICON_SHIELD_REGION_CREATE_FAILED");
    bool transferred = false;
    try {
      foreach (ShieldRect shield in shields) {
        int left = Math.Max(0, shield.x - windowRect.left);
        int top = Math.Max(0, shield.y - windowRect.top);
        int right = Math.Min(windowWidth, shield.x + shield.width - windowRect.left);
        int bottom = Math.Min(windowHeight, shield.y + shield.height - windowRect.top);
        if (right <= left || bottom <= top) continue;
        IntPtr cut = CreateRectRgn(left, top, right, bottom);
        if (cut == IntPtr.Zero)
          throw new InvalidOperationException("DESKTOP_ICON_SHIELD_REGION_CREATE_FAILED");
        try {
          if (CombineRgn(region, region, cut, RGN_DIFF) == 0)
            throw new InvalidOperationException("DESKTOP_ICON_SHIELD_REGION_COMBINE_FAILED");
        } finally {
          DeleteObject(cut);
        }
      }
      if (SetWindowRgn(_listView, region, true) == 0)
        throw new InvalidOperationException("DESKTOP_ICON_SHIELD_APPLY_FAILED");
      transferred = true;
      _shieldRegionApplied = true;
      RefreshListViewComposition();
    } finally {
      if (!transferred) DeleteObject(region);
    }
  }

  private static void RestoreListViewShieldRegion() {
    if (!_shieldRegionApplied) return;
    ...
    if (!BoundTargetIdentityMatches())
      throw new InvalidOperationException("DESKTOP_ICON_SHIELD_IDENTITY_UNCERTAIN");
    if (SetWindowRgn(_listView, IntPtr.Zero, true) == 0)
      throw new InvalidOperationException("DESKTOP_ICON_SHIELD_RESTORE_FAILED");
    _shieldRegionApplied = false;
    RefreshListViewComposition();
  }
  ...
  private static void ApplyPendingControls() {
    ...
    lock (StateLock) {
      ...
      shieldPending = _shieldRequestPending;
      shieldSequence = _shieldRequestSequence;
    ...
    lock (StateLock) {
      ...
      if (shieldPending && _shieldRequestSequence == shieldSequence)
        _shieldRequestPending = false;
    ...
    try {
      while (true) {
        ...
        if (String.Equals(fields[0], "L", StringComparison.OrdinalIgnoreCase)) {
          ...
          QueueRefresh();
        } else if (String.Equals(fields[0], "S", StringComparison.OrdinalIgnoreCase)) {
          System.Collections.Generic.List<ShieldRect> shields =
            ParseShieldRects(fields.Length > 2 ? fields[2] : "");
          lock (StateLock) {
            _requestedShields = shields;
            _shieldRequestPending = true;
            _shieldRequestSequence = sequence;
            _controlSequence = Math.Max(_controlSequence, sequence);
          }
          QueueRefresh();
        }
  ...
  public static void Run(int debounceMs, int rebindMs, int ownerProcessId,
    ...
    Exception shieldRestoreFailure = null;
    try {
      BindExpectedTarget();
      ApplyAndEmit(true);
      ...
      try { RestoreListViewShieldRegion(); }
      catch (Exception error) { shieldRestoreFailure = error; }
    ...
    Exception restoreFailure = shieldRestoreFailure ?? backgroundRestoreFailure ?? transparencyRestoreFailure ?? visibilityRestoreFailure;
...
function startNativeDesktopIconLayer(options = {}) {
  ...
  const updateShields = (rects = [], timeoutMs) => {
    const payload = (Array.isArray(rects) ? rects : []).slice(0, 64)
      .map(normalizeRect)
      .filter(Boolean)
      .map((rect) => [rect.x, rect.y, rect.width, rect.height].join(','))
      .join(';');
    return sendControl('S', payload, timeoutMs);
  };
