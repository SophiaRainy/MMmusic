...
const BILIBILI_LOGIN_PARTITION = 'persist:mineradio-bilibili-login';
...
const BILIBILI_LOGIN_COOKIE_PRIORITY = [
  'SESSDATA',
  'bili_jct',
  'DedeUserID',
  'DedeUserID__ckMd5',
...
function bilibiliCookieHasLogin(cookieText) {
  const obj = parseCookieHeader(cookieText);
  return !!(obj.SESSDATA && obj.DedeUserID);
}
...
function isBilibiliCookieDomain(domain) {
  ...
  return normalized === 'bilibili.com' || normalized.endsWith('.bilibili.com') ||
    normalized === 'biligame.com' || normalized.endsWith('.biligame.com');
}
...
async function readBilibiliLoginCookieHeader(cookieSession) {
  const cookies = await cookieSession.cookies.get({});
  return buildCookieHeaderFor(cookies, isBilibiliCookieDomain, BILIBILI_LOGIN_COOKIE_PRIORITY);
}
...
async function openBilibiliMusicLoginWindow(owner) {
  const cookieSession = session.fromPartition(BILIBILI_LOGIN_PARTITION);
  const initialCookie = await readBilibiliLoginCookieHeader(cookieSession);
  if (bilibiliCookieHasLogin(initialCookie)) return { ok: true, cookie: initialCookie, reused: true };
  ...
  return new Promise((resolve) => {
    ...
    const loginWindow = new BrowserWindow({
      ...
      icon: APP_ICON_ICO,
      webPreferences: {
        partition: BILIBILI_LOGIN_PARTITION,
        contextIsolation: true,
        nodeIntegration: false,
        sandbox: true,
      },
    ...
    const checkCookies = async () => {
      try {
        const cookie = await readBilibiliLoginCookieHeader(cookieSession);
        if (bilibiliCookieHasLogin(cookie)) finish({ ok: true, cookie });
      } catch (error) {
        console.warn('Bilibili login cookie check failed:', error.message);
      }
    };
    ...
    loginWindow.webContents.setWindowOpenHandler(({ url }) => {
      try {
        ...
        const hostname = parsed.hostname.toLowerCase();
        if (parsed.protocol === 'https:' && (hostname === 'bilibili.com' || hostname.endsWith('.bilibili.com'))) {
          loginWindow.loadURL(url).catch((error) => console.warn('Bilibili login popup navigation failed:', error.message));
        } else if (/^https?:\/\//i.test(url)) {
          shell.openExternal(url).catch(() => {});
        }
      } catch (_) {}
    ...
    loginWindow.on('closed', async () => {
      ...
      try {
        const cookie = await readBilibiliLoginCookieHeader(cookieSession);
        try { await cookieSession.flushStorageData(); } catch (_) {}
        resolve(bilibiliCookieHasLogin(cookie)
          ? { ok: true, cookie }
          : { ok: false, cancelled: true, message: '哔哩哔哩登录窗口已关闭' });
      } catch (error) {
        resolve({ ok: false, error: error.message || '哔哩哔哩登录窗口已关闭' });
      }
    });

    pollTimer = setInterval(checkCookies, 1000);
    loginWindow.loadURL(BILIBILI_LOGIN_URL).catch((error) => finish({ ok: false, error: error.message }));
  });
}

async function clearBilibiliMusicLoginSession() {
  const cookieSession = session.fromPartition(BILIBILI_LOGIN_PARTITION);
  await cookieSession.clearStorageData({
    storages: ['cookies', 'localstorage', 'indexdb', 'cachestorage'],
  });
  return { ok: true, provider: 'bilibili' };
}
...
async function clearAllProviderLoginState(reason) {
  ...
  const results = await Promise.allSettled([
    ...
    clearBilibiliMusicLoginSession(),
  ]);
...
function loginCookieExportMeta(provider) {
  ...
  const entries = {
    ...
    bilibili: { label: '哔哩哔哩', files: [process.env.BILIBILI_COOKIE_FILE, path.join(userData, '.bilibili-cookie')] },
  };
...
ipcMain.handle('bilibili-music-open-login', async (event) => {
  if (!loginEasterEggGate.isUnlocked()) return loginEasterEggLockedResult();
  return openBilibiliMusicLoginWindow(getSenderWindow(event));
});

ipcMain.handle('bilibili-music-clear-login', async () => {
  return clearBilibiliMusicLoginSession();
});
...
function configureLocalServerEnvironment(port) {
  ...
  process.env.BILIBILI_COOKIE_FILE = path.join(STABLE_USER_DATA_PATH, '.bilibili-cookie');
  process.env.BILIBILI_CONFIG_FILE = path.join(STABLE_USER_DATA_PATH, '.bilibili-config.json');
