...
test('native layered guard puts the complete main HWND below Explorer icons and restores all snapshots', () => {
  ...
  assert.match(script, /SetWindowRgn\(_listView, region, true\)/);
  assert.match(script, /CombineRgn\(region, region, cut, RGN_DIFF\)/);
  assert.match(script, /RestoreListViewShieldRegion\(\)/);
  assert.doesNotMatch(script,
    /EnableWindow|LVM_SETTEXTBKCOLOR|GetCursorPos|SetCursorPos|SendInput|WM_MOUSEMOVE|Mineradio Desktop Icon Lock Shield|CreateWindowEx/);
});
...
test('external guard keeps WMI crash recovery but runs PowerShell under headless System32 conhost', () => {
  ...
  assert.match(source, /const updateShields = \(rects = \[\], timeoutMs\) =>/);
  assert.match(source, /sendControl\('S', payload, timeoutMs\)/);
});
...
test('native layered wrapper exposes explicit icon visibility and terminal diagnostics', async () => {
  ...
  child.stdin.on('data', (chunk) => {
    ...
    const control = text.match(/^([LZVS])\|(\d+)\|([^\r\n]*)/m);
    if (control) {
  ...
  assert.equal((await watcher.updateShields([{ x: 10, y: 20, width: 30, height: 40 }])).controlSequence, 1);
  assert.equal((await watcher.setLocked(true)).controlSequence, 2);
  ...
  assert.equal((await watcher.ensureOrder()).controlSequence, 4);
