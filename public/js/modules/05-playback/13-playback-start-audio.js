...
async function resolveAlbumGaplessPlaybackData(song) {
  ...
  if (playbackProvider === 'bilibili') {
    return apiJson('/api/bilibili/song/url?id=' + encodeURIComponent(song.id || song.providerSongId || '') +
      '&bvid=' + encodeURIComponent(song.bvid || '') +
      '&aid=' + encodeURIComponent(song.aid || '') +
      '&cid=' + encodeURIComponent(song.cid || '') +
      '&page=' + encodeURIComponent(song.page || 1) +
      qualityParam, { timeoutMs: 15000 });
  }
...
async function playQueueAt(idx, opts) {
  ...
  try {
    ...
    try {
      ...
      var isBilibiliPlayback = playbackProvider === 'bilibili';
      ...
      if (albumGaplessHandoff) {
        ...
        data = await apiJson('/api/spotify/song/url?id=' + encodeURIComponent(song.id || song.providerSongId || song.spotifyId || '') +
          ...
          '&uri=' + encodeURIComponent(song.spotifyUri || song.uri || '') +
          qualityParam, { timeoutMs: 9000 });
      } else if (isBilibiliPlayback) {
        data = await apiJson('/api/bilibili/song/url?id=' + encodeURIComponent(song.id || song.providerSongId || '') +
          '&bvid=' + encodeURIComponent(song.bvid || '') +
          '&aid=' + encodeURIComponent(song.aid || '') +
          '&cid=' + encodeURIComponent(song.cid || '') +
          '&page=' + encodeURIComponent(song.page || 1) +
      ...
      if (!opts.startupAutoplay && !isQQPlayback && qualityDowngraded) {
        showSourceFallbackNotice((isKugouPlayback ? '酷狗' : (isQishuiPlayback ? '汽水' : (isBilibiliPlayback ? 'B站' : '网易云'))) + '音质自动降级', '请求 ' + playbackQualityLabel(requestedQuality, playbackProvider) + '，实际播放 ' + resolvedQualityText + '。');
