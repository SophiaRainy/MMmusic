...
function songProviderKey(song) {
  if (song && (song.provider === 'bilibili' || song.source === 'bilibili' || song.type === 'bilibili' || song.bvid)) return 'bilibili';
...
function songSourceTagHtml(song, opts) {
  ...
  var key = /^(netease|qq|kugou|qishui|spotify|bilibili)$/.test(String(rawKey || '')) ? String(rawKey) : songProviderKey(song);
  var label = key === 'qq' ? 'QQ' : (key === 'kugou' ? 'KG' : (key === 'qishui' ? 'QS' : (key === 'spotify' ? 'SP' : (key === 'bilibili' ? 'BL' : 'NE'))));
  if (opts.switcher) {
...
function searchResultMetaText(song) {
  ...
  if (songProviderKey(song) === 'bilibili' && !song.playable) bits.push('B站视频音频不可用');
