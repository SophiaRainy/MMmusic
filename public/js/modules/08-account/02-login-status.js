...
function normalizeBilibiliLoginStatus(info) {
  var fallback = { provider: 'bilibili', loggedIn: false, preview: false, nickname: '哔哩哔哩', userId: '', avatar: '', vipType: 0, vipLevel: 'none', isVip: false, isSvip: false, stale: false, playbackKeyReady: true };
  var loggedIn = !!(info && info.loggedIn);
  return Object.assign({}, fallback, info || {}, {
    provider: 'bilibili',
    loggedIn: loggedIn,
    nickname: info && info.nickname || fallback.nickname,
    userId: info && (info.userId || info.mid) || '',
    avatar: info && info.avatar || '',
    vipType: Number(info && info.vipType || 0) || 0,
    vipLevel: info && info.vipLevel || 'none',
    isVip: !!(info && info.isVip),
    isSvip: false,
    stale: !!(info && info.stale),
    playbackKeyReady: true
...
async function refreshBilibiliLoginStatus() {
  try {
    var info = await apiJson('/api/bilibili/login/status?t=' + Date.now());
    var prevLogged = !!bilibiliLoginStatus.loggedIn;
    bilibiliLoginStatus = normalizeBilibiliLoginStatus(info);
    if (!bilibiliLoginStatus.loggedIn) {
      if (prevLogged || bilibiliLoginWasLoggedIn) showToast(bilibiliLoginStatus.stale ? '哔哩哔哩登录已失效' : '哔哩哔哩已退出');
      bilibiliPlaylists = [];
      userPlaylists = userPlaylists.filter(function (pl) { return pl.provider !== 'bilibili'; });
      playlistCatalogRevision += 1;
      homeDiscoverState.loaded = false;
    } else if (!userPlaylists.some(function (pl) { return pl && pl.provider === 'bilibili'; })) {
      homeDiscoverState.loaded = false;
      homeDiscoverState.loggedIn = true;
      refreshUserPlaylists(true);
    }
    bilibiliLoginWasLoggedIn = !!bilibiliLoginStatus.loggedIn;
    if (!hasPlatformLogin(activeAccountProvider)) activeAccountProvider = firstLoggedProvider();
    renderUserBtn();
    return bilibiliLoginStatus;
  } catch (e) {
    console.warn('Bilibili login status failed:', e);
    bilibiliLoginStatus = normalizeBilibiliLoginStatus(null);
    renderUserBtn();
    return bilibiliLoginStatus;
  }
}

function startBilibiliLoginStatusAutoRefresh() {
  if (bilibiliLoginAutoRefreshTimer) clearInterval(bilibiliLoginAutoRefreshTimer);
  bilibiliLoginAutoRefreshTimer = setInterval(function () {
    refreshBilibiliLoginStatus().catch(function (e) { console.warn('Bilibili login auto refresh failed:', e); });
  }, 45000);
}
