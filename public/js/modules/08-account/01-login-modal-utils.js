...
var ACCOUNT_PROVIDER_KEYS = ['netease', 'qq', 'kugou', 'qishui', 'spotify', 'bilibili'];
...
function platformMeta(provider) {
  ...
  if (provider === 'bilibili') return { key: 'bilibili', short: 'BL', label: '哔哩哔哩', app: '哔哩哔哩 App', dot: 'bilibili' };
  return { key: 'netease', short: 'NE', label: '网易云音乐', app: '网易云音乐 App', dot: 'netease' };
}
function platformStatus(provider) {
  if (provider === 'bilibili') return bilibiliLoginStatus;
...
function hasAnyPlatformLogin() {
  return hasPlatformLogin('netease') || hasPlatformLogin('qq') || hasPlatformLogin('kugou') || hasPlatformLogin('qishui') || hasPlatformLogin('spotify') || hasPlatformLogin('bilibili');
}
...
function providerAvatarSrc(provider, status) {
  ...
  var fill = provider === 'qq' ? '#bfd66b' : (provider === 'kugou' ? '#56e0ff' : (provider === 'qishui' ? '#45d68f' : (provider === 'spotify' ? '#1ed760' : (provider === 'bilibili' ? '#fb7299' : '#d95b67'))));
  var bg = provider === 'qq' ? '#11150b' : (provider === 'kugou' ? '#071722' : (provider === 'qishui' ? '#071a12' : (provider === 'spotify' ? '#06140a' : (provider === 'bilibili' ? '#1b0b12' : '#180b0f'))));
...
function providerAccountIdentity(provider, status) {
  ...
  var syntheticPrefixes = [meta.label, meta.short, provider, 'QQ 音乐', 'QQ', '酷狗音乐', '酷狗', '汽水音乐', '网易云音乐', '网易云', 'Spotify', '哔哩哔哩', 'B站', 'Bilibili']
