function loggedProviderCount() {
  return ['netease', 'qq', 'kugou', 'qishui', 'spotify', 'bilibili'].filter(function (key) { return hasPlatformLogin(key); }).length;
}
function updateUserModalUi() {
  ...
  var addBilibili = document.getElementById('account-add-bilibili');
  var bilibiliFolderActions = document.getElementById('bilibili-folder-actions');
  if (chip) {
  ...
  if (vipEl) {
    ...
    } else if (activeAccountProvider === 'bilibili') {
      vipEl.textContent = 'UID: ' + ((st && st.userId) || '-') + '  /  自动同步音乐命名收藏夹，可手动添加收藏夹';
      vipEl.style.color = 'rgba(251,114,153,0.86)';
    } else {
  ...
  ['netease', 'qq', 'kugou', 'qishui', 'spotify', 'bilibili', 'both'].forEach(function (key) {
  ...
  if (addBilibili) addBilibili.textContent = hasPlatformLogin('bilibili') ? '查看哔哩哔哩' : '连接哔哩哔哩';
  if (bilibiliFolderActions) bilibiliFolderActions.style.display = activeAccountProvider === 'bilibili' && hasPlatformLogin('bilibili') ? '' : 'none';
  if (logoutBtn) logoutBtn.textContent =
    ...
    (activeAccountProvider === 'spotify' ? '退出 Spotify' :
    (activeAccountProvider === 'bilibili' ? '退出哔哩哔哩' : '退出网易云'))));
...
function setActiveAccountProvider(provider) {
  provider = provider === 'qq' ? 'qq' : (provider === 'kugou' ? 'kugou' : (provider === 'qishui' ? 'qishui' : (provider === 'spotify' ? 'spotify' : (provider === 'bilibili' ? 'bilibili' : 'netease'))));
...
async function addBilibiliFavoriteFolder() {
  var input = document.getElementById('bilibili-folder-input');
  var button = document.getElementById('bilibili-folder-add-btn');
  var value = input ? input.value.trim() : '';
  if (!value) {
    showToast('请输入 B站收藏夹 ID 或链接');
    return;
  }
  if (button) {
  ...
  try {
    var result = await apiJson('/api/bilibili/playlist/manual', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ value: value })
    });
    if (!result || !result.ok) throw new Error(result && (result.message || result.error) || '添加收藏夹失败');
    if (input) input.value = '';
    await refreshUserPlaylists(true);
    showToast('已添加 B站收藏夹: ' + (result.folder && result.folder.name || result.folder && result.folder.id || ''));
  } catch (e) {
    showToast(e && e.message ? e.message : '添加 B站收藏夹失败');
  } finally {
    if (button) {
...
function resetAllProviderRendererLoginState() {
  ...
  bilibiliLoginStatus = { provider: 'bilibili', loggedIn: false, preview: false, nickname: '哔哩哔哩', userId: '', avatar: '', vipType: 0, vipLevel: 'none', isVip: false, isSvip: false, playbackKeyReady: true };
...
async function logoutActiveAccount() {
  if (activeAccountProvider === 'bilibili') {
    try { await apiJson('/api/bilibili/logout'); } catch (e) { }
    try {
      if (window.desktopWindow && typeof window.desktopWindow.clearBilibiliMusicLogin === 'function') {
        await window.desktopWindow.clearBilibiliMusicLogin();
      }
    } catch (e) { }
    bilibiliLoginStatus = { provider: 'bilibili', loggedIn: false, preview: false, nickname: '哔哩哔哩', userId: '', avatar: '', vipType: 0, vipLevel: 'none', isVip: false, isSvip: false, playbackKeyReady: true };
    bilibiliPlaylists = [];
    userPlaylists = userPlaylists.filter(function (pl) { return pl.provider !== 'bilibili'; });
    playlistCatalogRevision += 1;
    dualAccountMode = false;
    activeAccountProvider = firstLoggedProvider();
    renderUserBtn();
    safeShelfRebuild('bilibili-logout');
...
async function doLogout() {
  ...
  activeAccountProvider = firstLoggedProvider();
  userPlaylists = qqPlaylists.concat(kugouPlaylists || [], qishuiPlaylists || [], spotifyPlaylists || [], bilibiliPlaylists || []);
  playlistCatalogRevision += 1;
  myPodcastCollections = [];
  myPodcastItems = {};
  likedSongMap = {};
