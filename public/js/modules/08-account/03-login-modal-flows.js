...
var LOGIN_WORKFLOW_PROVIDERS = ['netease', 'qq', 'kugou', 'qishui', 'spotify', 'bilibili'];
...
function normalizeLoginProviderKey(provider) {
  return provider === 'qq' ? 'qq' : (provider === 'kugou' ? 'kugou' : (provider === 'qishui' ? 'qishui' : (provider === 'spotify' ? 'spotify' : (provider === 'bilibili' ? 'bilibili' : 'netease'))));
}
...
function loginProviderOfficialModeText(provider) {
  ...
  if (provider === 'bilibili') return { title: '官网', sub: '弹出哔哩哔哩官方窗口' };
...
function setManualCookieOpenForProvider(provider, open) {
  ...
  else if (provider === 'bilibili') bilibiliManualCookieOpen = !!open;
}
function isManualCookieOpenForProvider(provider) {
  ...
  if (provider === 'bilibili') return !!bilibiliManualCookieOpen;
  return false;
}
...
function updateLoginProviderUi() {
  ...
  var isBilibili = loginProvider === 'bilibili';
  var isManualCookieProvider = isNetease || isQQ || isKugou || isBilibili;
  ...
  var bilibiliBtn = document.getElementById('login-provider-bilibili');
  ...
  if (isBilibili) {
    ...
    [neteaseBtn, qqBtn, kugouBtn, qishuiBtn, spotifyBtn].forEach(function (btn) { if (btn) btn.classList.remove('active'); });
    if (bilibiliBtn) bilibiliBtn.classList.add('active');
    if (title) title.textContent = '登录哔哩哔哩';
    if (desc) desc.innerHTML = '打开 <b>哔哩哔哩官方登录窗口</b> 扫码或登录，成功后自动同步名称包含音乐关键词的收藏夹。';
    var bilibiliManualOpen = isManualCookieOpenForProvider('bilibili');
    if (shell) {
      shell.classList.add('web-login-preview');
      shell.classList.remove('qq-preview', 'netease-preview');
    }
    if (qqPanel) qqPanel.classList.toggle('show', bilibiliManualOpen);
    if (qqCookieToggle) {
      qqCookieToggle.classList.add('show');
      qqCookieToggle.textContent = bilibiliManualOpen ? '收起导入' : 'Cookie 导入';
    }
    if (qqCookieInput) qqCookieInput.placeholder = 'SESSDATA=...; DedeUserID=...; bili_jct=...';
    if (qqCookieNote) qqCookieNote.textContent = '从 bilibili.com 的已登录会话导入。';
    if (qqCookieSaveBtn) {
      qqCookieSaveBtn.disabled = !!bilibiliWebLoginBusy;
      qqCookieSaveBtn.textContent = '保存 Cookie';
    }
    if (qqCard) {
      qqCard.style.display = '';
      qqCard.disabled = !!bilibiliWebLoginBusy;
      var blMark = qqCard.querySelector('b');
      var blLabel = qqCard.querySelector('span');
      if (blMark) blMark.textContent = 'BL';
      if (blLabel) blLabel.textContent = bilibiliWebLoginBusy ? '等待登录确认' : '打开官方登录窗口';
    }
    if (st) {
      st.className = 'preview';
      st.textContent = bilibiliLoginStatus.loggedIn
        ? ('已连接哔哩哔哩 · ' + (bilibiliLoginStatus.nickname || bilibiliLoginStatus.userId || ''))
        : '点击“登录”打开哔哩哔哩官方窗口';
    }
    if (refreshBtn) {
      refreshBtn.disabled = !!bilibiliWebLoginBusy;
      refreshBtn.textContent = bilibiliWebLoginBusy ? '等待登录…' : (bilibiliLoginStatus.loggedIn ? '刷新状态' : '登录');
      refreshBtn.onclick = bilibiliLoginStatus.loggedIn ? refreshQr : openBilibiliWebLogin;
    }
    updateLoginNodeGraphUi();
    return;
  }
  ...
  if (bilibiliBtn) bilibiliBtn.classList.toggle('active', false);
...
async function refreshQr() {
  ...
  if (loginProvider === 'bilibili') {
    qrKey = null;
    var bilibiliStatus = document.getElementById('qr-status');
    var bilibiliImg = document.getElementById('qr-img');
    if (bilibiliImg) bilibiliImg.src = '';
    var bilibiliInfo = await refreshBilibiliLoginStatus();
    if (!isLoginRefreshCurrent(refreshProvider, refreshSeq)) return;
    updateLoginProviderUi();
    if (bilibiliStatus) {
      bilibiliStatus.textContent = bilibiliInfo && bilibiliInfo.loggedIn
        ? ('已连接哔哩哔哩 · ' + (bilibiliInfo.nickname || bilibiliInfo.userId || ''))
        : '点击“登录”打开哔哩哔哩官方窗口';
      bilibiliStatus.className = 'preview';
    }
    return;
  }
...
function openProviderWebLogin() {
  ...
  if (loginProvider === 'bilibili') return openBilibiliWebLogin();
  return openNeteaseWebLogin();
}
async function openBilibiliWebLogin() {
  if (bilibiliWebLoginBusy) return;
  var statusEl = document.getElementById('qr-status');
  var api = window.desktopWindow;
  if (!api || !api.isDesktop || typeof api.openBilibiliMusicLogin !== 'function') {
    bilibiliManualCookieOpen = true;
    updateLoginProviderUi();
    if (statusEl) { statusEl.textContent = '当前环境不支持自动网页登录，可使用 Cookie 导入。'; statusEl.className = 'fail'; }
    return;
  }
  bilibiliWebLoginBusy = true;
  updateLoginProviderUi();
  if (statusEl) { statusEl.textContent = '已打开哔哩哔哩窗口，请完成官方登录…'; statusEl.className = 'preview'; }
  try {
    var result = await api.openBilibiliMusicLogin();
    if (!result || !result.ok || !result.cookie) {
      throw new Error((result && (result.message || result.error)) || '哔哩哔哩登录未完成');
    }
    if (statusEl) { statusEl.textContent = '正在同步哔哩哔哩账号和音乐收藏夹…'; statusEl.className = 'preview'; }
    var info = await apiJson('/api/bilibili/login/cookie', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ cookie: result.cookie })
    });
    if (!info || !info.loggedIn) throw new Error((info && (info.message || info.error)) || '哔哩哔哩会话不可用');
    bilibiliLoginStatus = normalizeBilibiliLoginStatus(info);
    activeAccountProvider = 'bilibili';
    bilibiliManualCookieOpen = false;
    renderUserBtn();
    await refreshUserPlaylists(true);
    if (statusEl) { statusEl.textContent = '哔哩哔哩收藏夹已接入'; statusEl.className = 'scan'; }
    offerLoginCookieExport('bilibili', info);
    setTimeout(function () {
      closeLoginModal();
      showToast('哔哩哔哩已登录: ' + (info.nickname || info.userId || ''));
    }, 420);
  } catch (e) {
    if (statusEl) { statusEl.textContent = e && e.message ? e.message : '哔哩哔哩登录失败'; statusEl.className = 'fail'; }
  } finally {
    bilibiliWebLoginBusy = false;
    updateLoginProviderUi();
  }
}
...
async function submitQQCookieLogin() {
  ...
  if (loginProvider === 'bilibili') return submitBilibiliCookieLogin();
  var isKugou = loginProvider === 'kugou';
...
async function submitBilibiliCookieLogin() {
  ...
  var cookie = input ? input.value.trim() : '';
  if (!cookie) {
    if (statusEl) { statusEl.textContent = '先粘贴哔哩哔哩 Cookie'; statusEl.className = 'fail'; }
    return;
  }
  qqCookieBusy = true;
  if (saveBtn) saveBtn.classList.add('busy');
  if (statusEl) { statusEl.textContent = '正在保存哔哩哔哩会话…'; statusEl.className = 'preview'; }
  try {
    var info = await apiJson('/api/bilibili/login/cookie', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    renderUserBtn();
    await refreshUserPlaylists(true);
    if (statusEl) { statusEl.textContent = '哔哩哔哩会话和音乐收藏夹已保存'; statusEl.className = 'scan'; }
    offerLoginCookieExport('bilibili', info);
    setTimeout(function () {
      closeLoginModal();
      showToast('哔哩哔哩已登录: ' + (info.nickname || info.userId || ''));
    ...
    if (statusEl) { statusEl.textContent = e && e.message ? e.message : '哔哩哔哩会话保存失败'; statusEl.className = 'fail'; }
  } finally {
    qqCookieBusy = false;
    if (saveBtn) saveBtn.classList.remove('busy');
    updateLoginProviderUi();
