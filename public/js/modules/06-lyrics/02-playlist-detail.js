...
function normalizePlaylistProvider(provider) {
  if (provider === 'qq' || provider === 'kugou' || provider === 'qishui' || provider === 'spotify' || provider === 'bilibili') return provider;
  return 'netease';
}
function playlistProviderLabel(provider) {
  provider = normalizePlaylistProvider(provider);
  return provider === 'qq' ? 'QQ' : (provider === 'kugou' ? 'KG' : (provider === 'qishui' ? 'QS' : (provider === 'spotify' ? 'SP' : (provider === 'bilibili' ? 'BL' : 'NE'))));
}
...
function playlistPanelProviderId(provider, id) {
  ...
  if (provider === 'bilibili') return 'bilibili:' + id;
  return id;
}
...
function playlistTracksEndpoint(provider, id, params) {
  ...
  if (provider === 'bilibili') return '/api/bilibili/playlist/tracks?' + query;
...
function playlistPanelBuildVirtualEntries() {
  ...
  var labels = { netease: '网易云歌单', qq: 'QQ 音乐歌单', kugou: '酷狗音乐歌单', qishui: '汽水音乐歌单', spotify: 'Spotify 歌单', bilibili: '哔哩哔哩音乐收藏夹' };
  var order = ['netease', 'qq', 'kugou', 'qishui', 'spotify', 'bilibili'];
  var groups = { netease: [], qq: [], kugou: [], qishui: [], spotify: [], bilibili: [] };
