...
function playlistQueueSource(id) {
  ...
  if (raw.indexOf('bilibili:') === 0) return { provider: 'bilibili', id: raw.slice(9), requestId: raw };
  return { provider: 'netease', id: raw, requestId: raw };
}
function playlistQueuePageSize(provider, initial) {
  if (initial) return provider === 'kugou' || provider === 'qishui' || provider === 'bilibili' ? 40 : (provider === 'spotify' ? 96 : PLAYLIST_QUEUE_INITIAL_BATCH_SIZE);
  if (provider === 'kugou' || provider === 'qishui') return 50;
  if (provider === 'spotify') return 100;
  if (provider === 'bilibili') return 40;
