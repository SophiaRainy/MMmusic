...
function desktopIconShieldShelfRects(viewportWidth, viewportHeight) {
  ...
  var side = 'right';
  try {
    if (typeof shelfManager !== 'undefined' && shelfManager && shelfManager.getSide) {
      side = String(shelfManager.getSide() || side).toLowerCase();
    } else if (typeof fx !== 'undefined' && fx && String(fx.shelfSide || '').toLowerCase() === 'left') {
      side = 'left';
    }
  } catch (_) { }
  var left = side === 'left' ? 0 : viewportWidth - width;
  return [desktopIconShieldClippedRect({
    left: left,
...
function updateDesktopWallpaperRuntimeControls(status) {
  ...
  if (toggle) {
    ...
    toggle.title = !supported
      ? '当前系统不支持完整桌面模式'
      : (attaching ? '正在切换完整桌面模式' : '把完整 Mineradio 放到 Windows 桌面；双击空白桌面可隐藏或显示全部图标，Esc 退出');
  }
