...
function audioProxyHeadersFor(audioUrl, range) {
  ...
  try {
    ...
    if (host.includes('bilivideo.com') || host.includes('bilivideo.cn') || host.includes('hdslb.com')) {
      headers.Referer = 'https://www.bilibili.com/';
      const bilibiliCookie = getBilibiliCookie();
...
function audioContentTypeForUrl(audioUrl, upstreamType) {
  ...
  if (/\.(m4a|mp4|m4s)$/.test(pathname)) return 'audio/mp4';
...
const server = http.createServer(async (req, res) => {
  ...
  if (pn === '/api/platform/capabilities') {
    ...
    sendJSON(res, {
      ...
      bilibili: {
        playlists: true, likeRead: false, likeWrite: false, albumRead: false,
        albumCollect: false, commentsRead: false, commentsWrite: false,
        manualFolders: true, videoAudioPlayback: true, listenReport: false,
      },
    });
    return;
  }
  ...
  if (pn === '/api/bilibili/status' || pn === '/api/bilibili/login/status') {
    try {
      sendJSON(res, await handleBilibiliStatus(getBilibiliCookie()));
    } catch (err) {
      console.error('[BilibiliStatus]', err);
      sendJSON(res, { provider: 'bilibili', loggedIn: false, error: err.message }, 500);
    }
    return;
  }

  if (pn === '/api/bilibili/login/cookie') {
    try {
      if (req.method !== 'POST') {
        sendJSON(res, { provider: 'bilibili', loggedIn: false, error: 'METHOD_NOT_ALLOWED' }, 405);
        return;
      }
      const body = await readRequestBody(req);
      const cookie = String(body.cookie || body.data || body.text || '').trim();
      if (!bilibiliCookieHasLogin(cookie)) {
        sendJSON(res, { provider: 'bilibili', loggedIn: false, error: 'INVALID_BILIBILI_COOKIE', message: 'B站 Cookie 缺少 SESSDATA 或 DedeUserID' }, 400);
        return;
      }
      setBilibiliCookie(cookie);
      const status = await handleBilibiliStatus(cookie);
      if (!status.loggedIn) throw new Error(status.error || 'BILIBILI_LOGIN_INVALID');
      sendJSON(res, Object.assign({}, status, { saved: true }));
    } catch (err) {
      console.error('[BilibiliLoginCookie]', err);
      sendJSON(res, { provider: 'bilibili', loggedIn: false, error: err.message }, 500);
    }
    return;
  }

  if (pn === '/api/bilibili/logout') {
    clearBilibiliCookie();
    sendJSON(res, { provider: 'bilibili', loggedIn: false, ok: true });
    return;
  }

  if (pn === '/api/bilibili/user/playlists') {
    try {
      sendJSON(res, await handleBilibiliUserPlaylists(getBilibiliCookie()));
    } catch (err) {
      console.error('[BilibiliUserPlaylists]', err);
      sendJSON(res, { provider: 'bilibili', loggedIn: false, error: err.message, playlists: [] }, 500);
    }
    return;
  }

  if (pn === '/api/bilibili/playlist/tracks') {
    try {
      const id = url.searchParams.get('id') || '';
      const limit = Math.max(1, Math.min(40, parseInt(url.searchParams.get('limit') || '20', 10) || 20));
      const offset = Math.max(0, parseInt(url.searchParams.get('offset') || '0', 10) || 0);
      sendJSON(res, await handleBilibiliPlaylistTracks(id, { limit, offset }, getBilibiliCookie()));
    } catch (err) {
      console.error('[BilibiliPlaylistTracks]', err);
      sendJSON(res, { provider: 'bilibili', error: err.message, tracks: [] }, 500);
    }
    return;
  }

  if (pn === '/api/bilibili/playlist/manual') {
    try {
      if (req.method !== 'POST' && req.method !== 'DELETE') {
        sendJSON(res, { provider: 'bilibili', ok: false, error: 'METHOD_NOT_ALLOWED' }, 405);
        return;
      }
      const body = await readRequestBody(req);
      const value = body.id || body.url || body.value || '';
      const result = req.method === 'DELETE'
        ? removeBilibiliManualFolder(value)
        : await addBilibiliManualFolder(value, getBilibiliCookie());
      sendJSON(res, result);
    } catch (err) {
      console.error('[BilibiliManualFolder]', err);
      sendJSON(res, { provider: 'bilibili', ok: false, error: err.message, message: err.message }, 400);
    }
    return;
  }

  if (pn === '/api/bilibili/song/url') {
    try {
      sendJSON(res, await handleBilibiliSongUrl({
        id: url.searchParams.get('id') || '',
        bvid: url.searchParams.get('bvid') || '',
        aid: url.searchParams.get('aid') || '',
        cid: url.searchParams.get('cid') || '',
        page: url.searchParams.get('page') || '1',
        quality: url.searchParams.get('quality') || 'exhigh',
      }, getBilibiliCookie()));
    } catch (err) {
      console.error('[BilibiliSongUrl]', err);
      sendJSON(res, { provider: 'bilibili', url: '', playable: false, error: err.message }, 500);
    }
    return;
  }

  if (pn === '/api/bilibili/lyric') {
    sendJSON(res, { provider: 'bilibili', lyric: '', tlyric: '', yrc: '' });
    return;
  }
